...
type Dispositivo = { id: string; numero_legajo: string; email: string }
...
export function AdminDispositivosPage() {
  ...
  const crearMutation = useMutation({
    mutationFn: (numero_legajo: string) => api.post('/admin/dispositivos', { numero_legajo }),
    onSuccess: () => {
                ...
                <tr key={d.id} className={i < dispositivos.length - 1 ? 'border-b border-[#1a2540]' : ''}>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <code className="font-mono text-xs text-[#6b7a9c] truncate max-w-[200px]">{d.id}</code>
                      <button
                        onClick={() => copyId(d.id)}
                        className="text-[#3a4a6b] hover:text-[#39ff14] transition-colors shrink-0"
                      >
                        {copied === d.id
      ...
      <Modal open={showModal} onClose={() => setShowModal(false)} title="Nuevo dispositivo" size="sm">
        <div className="space-y-4">
          <div>
            ...
            <select className="input-field" value={legajo} onChange={e => setLegajo(e.target.value)}>
          ...
          <button
            onClick={() => crearMutation.mutate(legajo)}
            disabled={!legajo || crearMutation.isPending}
