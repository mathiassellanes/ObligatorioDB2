      ...
      <div className="flex gap-1 mb-6 bg-[#090f20] p-1 rounded-xl border border-[#1a2540]">
        {([['eventos', 'Eventos', Calendar], ['estadios', 'Estadios', Building2], ['reportes', 'Reportes', BarChart3]] as const).map(([key, label, Icon]) => (
          <button key={key} onClick={() => setTab(key)}
            className={`flex items-center gap-2 flex-1 py-2 rounded-lg text-sm font-display font-700 uppercase tracking-wide justify-center transition-all ${
    ...
    <div>
      ...
      {show && (
        <div className="card p-5 mb-4 border-[#39ff1430]">
          <h3 className="font-display font-700 text-sm uppercase tracking-widest text-[#39ff14] mb-4">Nuevo evento</h3>
        ...
        : eventos.map(ev => (
          <div key={ev.id} className="card p-4 mb-2 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="font-display font-800 text-sm uppercase">
      ...
      {isLoading ? [...Array(4)].map((_, i) => <div key={i} className="card h-14 animate-pulse" />) :
        estadios.map(e => (
          <div key={e.id} className="card p-4 flex items-center gap-3">
            ...
            <span className="font-display font-700 text-sm uppercase">{e.nombre}</span>
          </div>
        ))}
    </div>
  )
...
function ReportesTab() {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div>
        <h3 className="font-display font-800 text-sm uppercase tracking-widest text-[#6b7a9c] mb-3 flex items-center gap-2">
          <TrendingUp className="w-4 h-4 text-[#39ff14]" />Eventos más vendidos
        </h3>
        <div className="space-y-2">
          {eventos.map((ev, i) => (
            <div key={ev.id} className="card p-3 flex items-center gap-3">
              <span className="font-display font-900 text-[#39ff14] text-lg w-6">{i + 1}</span>
              <div className="flex-1 min-w-0">
                <div className="text-xs font-display font-700 uppercase truncate">
        ...
        <div className="space-y-2">
          {compradores.map((c, i) => (
            <div key={c.email} className="card p-3 flex items-center gap-3">
              <span className="font-display font-900 text-[#ffb800] text-lg w-6">{i + 1}</span>
              <div className="flex-1 min-w-0">
